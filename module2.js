...
function make_RouteList( txtStr, routeName, mapfit = 0 ){
  ...
  // track毎のラインと始終点を地図に追加、重複するtrkptを除去
	...
	for ( let i = 0; i < RouteList[routeId][1]; i++ ){
		...
		let DrawDat = draw_trackLine( RTid, latlon ); // line, icon 描画
...
// 編集／wptマーカーclickイベント(削除【make】【edit】、マーカー情報表示【wptEdit】)
...
// 編集マーカーDragイベント(マーカーの移動、LmarkerList/LmarkerIndexの更新、マーカー間ライン(青)の再描画)【make】【edit】
...
function trkLine_crkOpe( routeId, trckNumber, NpInfo ){
	switch (mode){
		case "divide": case "remove": case "Pinfo":
			...
			break;
		case "merge":
			if (Object.keys(ChoseRoute).length < 2 ){ drw_seleLine( routeId, "yellow" ); }
			break;
		case "decimate": case "reverse": case "NaChang": case "save": case "EleRepl":
			if (Object.keys(ChoseRoute).length === 0 ){
				drw_seleLine( routeId, "green" );
			}else{
				routeId = Object.keys(ChoseRoute)[0];
			}
			break;
		case "TmChang": 
			if (Object.keys(ChoseRoute).length === 0 ){
				if ( Bttn2Value() === "0" ){
					...
					ChoseTrack = trckNumber;
					drw_seleLineTrk( routeId, trckNumber, "green" );
				}
			}else{
				routeId = Object.keys(ChoseRoute)[0];
			}
		...
		case "delete":
			...
			return;
		case "wptEdit":
			if (Object.keys(ChoseRoute).length === 0 ){ place_wptMark( routeId ); }else{ return; }
			break;
	}
...
// 選択ルートのline描画、lineレイヤをChoseRouteに登録【divide, remove, Pinfo】
function drw_seleLine( routeId, lineColor ){
	let lineArr = [];
	for ( let i = 0; i < RouteList[routeId][1]; i++ ){
		let trksegTxt = TrksegTxt[routeId][ i ];
		let latlon = make_LatlonFmTrkTxt( trksegTxt );
		let choseLine = L.polyline(latlon, {title:routeId, color: lineColor, weight: 5, bubblingMouseEvents: false }).on('click', SelectRouteClick).addTo(mymap);
		lineArr.push(choseLine);
	}
	ChoseRoute[ routeId ] = lineArr.slice();
}

// 選択トラックのline描画、lineレイヤをChoseRouteに登録【TmChang】
function drw_seleLineTrk( routeId, trckNumber, lineColor ){
	let lineArr = [];
	let trksegTxt = TrksegTxt[routeId][ trckNumber-1 ];
	let latlon = make_LatlonFmTrkTxt( trksegTxt );
	let choseLine = L.polyline(latlon, {title:routeId, color: lineColor, weight: 5, bubblingMouseEvents: false }).on('click', SelectRouteClick).addTo(mymap);
	lineArr.push(choseLine);
	ChoseRoute[ routeId ] = lineArr.slice();
...
// ウェイポイント編集用のライン選択とマーカー設置【wptEdit】
...
// wpt編集時の選択ラインのクリックでマーカー追加【wptEdit】
function wtpLine_click(e){
	let Llatlngs = e.target._latlngs.slice();
	let latlons = [];  // クリックポイントに一番近いtrkptを求める
	for ( let i = 0; i < Llatlngs.length; i++ ){ latlons.push ( [ Llatlngs[ i ]["lat"], Llatlngs[ i ]["lng"] ] ); }
	let NpInfo = nearPt_trkLinArr( e.latlng.lat, e.latlng.lng, latlons );
	put_wptMark( NpInfo[0][0],  NpInfo[0][1] );
}

// wptマーカー追加【wptEdit】
function put_wptMark( lat, lon ){
	let mkName = "m" + markN;
	let PointMark = L.marker([ lat, lon ], {title:mkName }).on('click', MarkerClick).on('contextmenu', MarkerClickRt).addTo(mymap);
	LmarkerList[mkName] =  PointMark;
...
function replace_wptTxt(){
	...
	RouteList[routeId][2] = Object.keys( LmarkerIndex ).length;
...
function dsp_routeInfo( routeId, trckNumber, NpInfo ){
	...
	switch (mode){
		...
		case "edit":
			 ...
			 WrtMessage1( `選択ルート：${RouteList[routeId][0]}、選択トラック：${Track[routeId][trckNumber -1 ]}、ポイント数：${NumOfTrackPoit}` );
			break;
		case "merge":
			...
			if ( CRnumb === 0 ) { 
				...
				if (CRnumb === 1 ){
					WrtMessage1( `選択ルート1：${RouteList[CR[0]][0]}` );
					document.getElementById("trk1").value = RouteList[CR[0]][0];
				}else{
					WrtMessage1( `選択ルート1：${RouteList[CR[0]][0]}、選択ルート2：${RouteList[CR[1]][0]} ` );
		...
		case "decimate":
			...
			WrtMessage1( `選択ルート：${RouteList[routeId][0]}    ポイント数：${NumOfTotalPoit}` );
			if ( Object.keys(ChoseRoute).length === 0 ) { WrtMessage1( ""); }
			break;
		case "TmChang":
			...
			let trkNaNu = "(" + Track[routeId][trckNumber -1 ] + ")";
			if ( Bttn2Value() === "0" ){ trkNaNu = ""; }
			WrtMessage1( ` 選択ルート：${RouteList[routeId][0]}${trkNaNu}   ${DayStr} ${stTimStr} ～ ${DayEnd} ${edTimStr}` );
			if ( Object.keys(ChoseRoute).length === 0 ){ WrtMessage1(""); }
			break;
		case "reverse":
			WrtMessage1( `選択ルート：${RouteList[routeId][0]}` );
			document.getElementById("trk1").value = RouteList[routeId][0]+ "_Rev";
			if ( Object.keys(ChoseRoute).length === 0 ){ document.getElementById("trk1").value = ""; }
			break;
		case "NaChang":
			let TrackNamesTxt = "";
			let dispTrack = [], N1, N2;
			...
			WrtMessage1( `選択ルート：${RouteList[routeId][0]}、 トラック数 = ${RouteList[routeId][1]} ` );
			if ( Object.keys(ChoseRoute).length === 0 ){ 
