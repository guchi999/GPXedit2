...
// トラックライン(赤)とS/Gアイコンの描画 RTid=RxxTyy, LineLatlon=[ [lat,lon], .... ]
function draw_trackLine( RTid, LineLatlon ){
	let trackLine = L.polyline(LineLatlon, {title:RTid, color: 'red', weight: 5, bubblingMouseEvents: false }).on('click', TrackClick).addTo(mymap);
...
// モード変更時の初期化
function modeChange(){
	EditRtTr = {}; AddedMark = {}; ChoseTrack = 0; WrtMessage1("");  WrtMessage2("");
	...
	switch (mode){
		...
		case "save":
			...
			addRadioForm( "OParea2", 0, "selbttn2" , [ "元ファイル形式で保存", "最小サイズで保存" ]);
			addSaveLink("OParea3");
			break;
		case "Pinfo":
