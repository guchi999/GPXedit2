...
// kml 出力モード選択を追加する場所
	...
	for ( let i = 0; i < RouteList[routeId][1]; i++ ){ mymap.removeLayer( ChoseRoute[ routeId ][i] ); }
...
function exec_decimate(){
	if ( Object.keys(ChoseRoute).length === 0){ return; }
	let routeId = Object.keys(ChoseRoute)[0];
	let wptArr = make_wptArr( routeId );   // wpt削除回避の為のチェック用配列作成
	...
	delete_route( routeId );
	make_RouteList( routeTxt, RouteNam, 1 );
	routeTxt ="";
	modeChange();
	dsp_routeList();
}
...
function change_time(){
	...
	if (Bttn1Value() === "0" ){
		...
		if ( EE[0] != 0 ){
			alert( `トラック [${Track[ routeId ][ EE[1] ]}] の全部または一部に標高データがありません。\n作業選択の [標高置換] で標高を設定してください。` );
			return;
		}
	}
	if ( Bttn1Value() === "0" && Bttn2Value() === "0" ){ // 全トラック時間変更
	...
	if ( Bttn1Value() === "0" && Bttn2Value() === "1" ){ // 指定トラック時間変更
		let TKseg = TrksegTxt[ routeId ][ ChoseTrack -1 ];
		let timeArr = make_timeArr( make_LatlonFmTrkTxt( TKseg ), make_EleFmTrkTxt( TKseg ), startTime, endTime );
		TKseg = SegTimeTxt_change( TKseg, timeArr );
		TrksegTxt[ routeId ][ ChoseTrack -1 ] = TKseg;
	}
	if (Bttn1Value() === "1" ){
	...
	if ( Bttn1Value() === "1" && Bttn2Value() === "0" ){ // 全トラック時間シフト
		let TKseg = unify_track( routeId )[1];
	...
	if ( Bttn1Value() === "1" && Bttn2Value() === "1" ){ // 指定トラック時間シフト
		let TKseg = TrksegTxt[ routeId ][ ChoseTrack -1 ];
		TKseg = segTime_shuft( TKseg, startTime );
		TrksegTxt[ routeId ][ ChoseTrack -1 ] = TKseg;
	}
